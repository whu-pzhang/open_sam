...
class LoRA_qkv(nn.Module):
    ...
    def __init__(self, qkv, linear_a_q: nn.Module, linear_b_q: nn.Module,
...
class LoRASAM(nn.Module):
    ...
    def __init__(self, sam_model, rank: int, lora_layer=None):
        ...
        for t_layer_i, blk in enumerate(sam_model.image_encoder.blocks):
            ...
            blk.attn.qkv = LoRA_qkv(w_qkv_linear, w_a_linear_q, w_b_linear_q,
    ...
    def save_lora_parameters(self, filename: str):
        ...
        a_tensors = {
            f"w_a_{i:03d}": self.A_weights[i].weight
            for i in range(num_layer)
        }
        b_tensors = {
            f"w_b_{i:03d}": self.B_weights[i].weight
            for i in range(num_layer)
        }
