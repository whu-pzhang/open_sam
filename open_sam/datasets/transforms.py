...
class GenerateSAMPrompt(BaseTransform):
    ...
    def transform(self, results):
        ...
        if gt_seg_map is not None:
            propmts = self.generate_prompt(gt_seg_map, self.max_instances,
    ...
    def generate_prompt(seg_map,
        ...
        # if len(masks) == 0:
        #     masks, point_coords, boxes = None, None, None
        # else:
...
class PackSamInputs(BaseTransform):
    ...
    def transform(self, results):
        ...
        point_labels = torch.ones(point_coords.shape[:2], dtype=torch.uint8)
        ...
        data_sample.gt_instances = InstanceData(**gt_instances_data)
        # ---
