...
def is_box_near_crop_edge(boxes: torch.Tensor,
    ...
    image."""
    crop_box_torch = torch.as_tensor(crop_box,
                                     dtype=torch.float,
                                     device=boxes.device)
    orig_box_torch = torch.as_tensor(orig_box,
                                     dtype=torch.float,
                                     device=boxes.device)
    boxes = uncrop_boxes_xyxy(boxes, crop_box).float()
    near_crop_edge = torch.isclose(boxes,
                                   crop_box_torch[None, :],
                                   atol=atol,
                                   rtol=0)
    near_image_edge = torch.isclose(boxes,
                                    orig_box_torch[None, :],
                                    atol=atol,
                                    rtol=0)
...
def batched_mask_to_box(masks: torch.Tensor) -> torch.Tensor:
    ...
    in_width_coords = in_width * torch.arange(w,
                                              device=in_width.device)[None, :]
