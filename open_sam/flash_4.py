...
def _autotune(configs, function):
    ...
    def benchmark_torch_function_in_microseconds(f, *args, **kwargs):
        try:
            f(*args, **kwargs)
            t0 = benchmark.Timer(stmt="f(*args, **kwargs)",
                                 globals={
                                     "args": args,
                                     "kwargs": kwargs,
                                     "f": f
...
def _save_best_configs(best_configs):
    ...
    if not device_name.startswith('NVIDIA A100'):
        ...
        print(
            f"Storing configs for {device_name} locally under {saved_configs}")
...
def _create_best_configs_key(q, k, v, rel_h_w, o):
    key = (q.size(), k.size(), v.size(), rel_h_w.size(), o.size(), q.stride(),
           k.stride(), v.stride(), rel_h_w.stride(), o.stride())
    return key
...
def _attention_rel_h_rel_w_kernel_aligned(q, k, v, rel_h_w, sm_scale):
    ...
    if key not in BEST_CONFIGS:
        print("key ", key,
              " not found. Running autotune. This might take a while.")
        import functools
        import itertools
        configs = []
        for (BLOCK_M, BLOCK_N, num_warps) in itertools.product([64, 128],
                                                               [64, 128],
                                                               [1, 2, 4, 8]):
        ...
        best, best_config = _autotune(
            configs,
            functools.partial(_attention_rel_h_rel_w_kernel_aligned_device, q,
                              k, v, rel_h_w, sm_scale, o))
        BEST_CONFIGS[key] = best_config
        print("Found best_config ", best_config, " with time ", best,
              " for key ", key)
    ...
    _attention_rel_h_rel_w_kernel_aligned_device(q, k, v, rel_h_w, sm_scale, o,
                                                 best_config[0],
                                                 best_config[1],
                                                 best_config[2],
                                                 best_config[3])
...
USE_CUSTOM_KERNEL = bool(
    int(os.environ.get('SEGMENT_ANYTHING_FAST_USE_FLASH_4', 1)))
...
def _attention_rel_h_rel_w(q_, k_, v_, rel_h_, rel_w_):
    ...
    def kernel_guards(q_, k_, v_):
        ...
        ) and q_.dtype == k_.dtype and k_.dtype == v_.dtype and USE_CUSTOM_KERNEL

    # vit_b and vit_l
    if q_size_2_padded == 0 and q_.size(-1) == 64 and kernel_guards(
            q_, k_, v_):
        rel_h_w = torch.cat([rel_h_.squeeze(-1), rel_w_.squeeze(-2)], dim=-1)
        o = torch.ops.customflash.custom_flash_aligned(q_, k_, v_, rel_h_w,
                                                       sm_scale)
        if o.numel() > 0:
            return o
    # vit_h
    if q_size_2_padded == 0 and q_.size(-1) == 80 and kernel_guards(
            q_, k_, v_):
        ...
        rel_h_w = torch.cat([rel_h_.squeeze(-1), rel_w_.squeeze(-2)], dim=-1)
        o = torch.ops.customflash.custom_flash_aligned(q, k, v, rel_h_w,
                                                       sm_scale)
        if o.numel() > 0:
            return o[:, :, :, :80]
    attn_bias = (rel_h_ + rel_w_).view(q_.size(0), q_.size(1), rel_h_.size(2),
                                       rel_h_.size(3) * rel_w_.size(4))
